...
// convert the headers array into an object
Request.prototype.preProcessHeaders = function() {
  this.headersRaw.forEach(function(header) {
    // "false" tells Caseless to not clobber values
    this.headers.set(header.name, header.value, false);
  }, this);

  if (this.headers.has('cf-railgun')) {
...
Request.prototype.processRailgunHeader = function() {
  ...
  var railgunHeader = this.headers.get('cf-railgun');
...
Request.prototype.servedByCloudFlare = function() {
  if (this.headers.has('server')) {
    return this.headers.get('server') === 'cloudflare-nginx';
  }
};

Request.prototype.servedByRailgun = function() {
  return this.headers.has('cf-railgun');
};
...
Request.prototype.getRayID = function() {
  return this.headers.get('cf-ray').split('-')[0];
};

Request.prototype.getCloudFlareLocationCode = function() {
  return this.headers.get('cf-ray').split('-')[1];
};
